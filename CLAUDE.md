...
| 문서 | 내용 |
|---|---|
| `docs/DESIGN.md` | **왜 이런 모양인지.** 결정 10개를 이유·버린 대안·되돌리면 생기는 일로 정리. 겉보기에 더 단순한 대안(전체 다운로드, 스트림 카피, 브라우저에서 처리)은 대부분 이미 검토하고 버린 것들이다. 구조를 바꾸기 전에 반드시 볼 것 |
| `docs/DESKTOP.md` | 설치형 데스크톱 앱으로 만드는 작업 지시서. 단계·함정·완료 판정 |
| `README.md` | 사용자용. 실행 방법, 구간 형식, 설정, 문제 해결 |
...
pytest                   # 101개. 유튜브 접속 없이 전부 실제로 돌린다
